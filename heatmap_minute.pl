...
print "Drive_name";
        for (my $i=1; $i <= $n_hours; $i++) {
                # print ",",$i;
		# print ",",scalar localtime(($minimum+(($i-1)*3600)-(3600*5)));
...
while (<$fh_dr>) {
	...
	$drive_name=$_;
	print $drive_name;
	open(my $fh_norm, '<', $filename) or die "cannot open file normalized.txt";
                        while (<$fh_norm>) {
				chomp;
				($start,$finish,$volume,$drive)=split/:/;
	...
	for (my $i=1; $i <= $n_hours; $i++) {
		$j=0;
		for ( $step=($minimum+(($i-1)*3600)) ; $step < ($minimum+($i*3600)) ; $step=$step+60 ) {
